                ...
                {levels.map((level, rowIndex) => (
                    <React.Fragment key={level}>
                        {Object.keys(kanjiData[level]).map((section, colIndex) => (
                            <div
                                key={`${level}-${section}`}
                                className={`border p-2 w-[30px] ml-1 h-[30px] text-center flex items-center justify-center cursor-pointer ${
                                    selectedLevel === level && selectedSection === section
                                        ? 'bg-blue-500'
                                        : 'bg-gray-300'
